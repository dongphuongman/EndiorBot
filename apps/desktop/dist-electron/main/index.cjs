...
function getStore() {
  if (!store) {
    store = new ElectronStore({
      name: "window-state",
      projectName: "endiorbot-desktop",
      // Required for electron-store v10+
      defaults: {
        windowState: {
          width: 1200,
          height: 800,
...
function createWindow(options) {
  mainWindow$1 = new electron.BrowserWindow(options);
  mainWindow$1.on("resize", () => saveWindowState());
  mainWindow$1.on("move", () => saveWindowState());
  mainWindow$1.on("maximize", () => saveWindowState());
  mainWindow$1.on("unmaximize", () => saveWindowState());
  mainWindow$1.on("close", () => saveWindowState());
...
function saveWindowState() {
  ...
  const isMaximized = mainWindow$1.isMaximized();
  const bounds = mainWindow$1.getBounds();
  if (!isMaximized) {
...
function getWindowIpcHandlers() {
  return {
    minimize: () => mainWindow$1 == null ? void 0 : mainWindow$1.minimize(),
    maximize: () => {
      if (mainWindow$1 == null ? void 0 : mainWindow$1.isMaximized()) {
        mainWindow$1.unmaximize();
      } else {
        mainWindow$1 == null ? void 0 : mainWindow$1.maximize();
      }
    },
    close: () => mainWindow$1 == null ? void 0 : mainWindow$1.close(),
    isMaximized: () => (mainWindow$1 == null ? void 0 : mainWindow$1.isMaximized()) ?? false,
    isFullScreen: () => (mainWindow$1 == null ? void 0 : mainWindow$1.isFullScreen()) ?? false,
    setFullScreen: (fullScreen) => mainWindow$1 == null ? void 0 : mainWindow$1.setFullScreen(fullScreen)
  };
}
...
async function loadCoreModules() {
  ...
  try {
    const core2 = await import("endiorbot");
    coreModules = {
      getSessionManager: core2.getSessionManager,
      createBudgetTracker: core2.createBudgetTracker,
      listCheckpoints: core2.listCheckpoints,
      loadCheckpoint: core2.loadCheckpoint,
      getPersistentFixLogger: core2.getPersistentFixLogger
    };
    return coreModules;
  } catch (error2) {
    console.warn("[IPC] Core modules not available, using fallbacks:", error2);
    return null;
  }
}
let budgetTracker = null;
let fixLogger = null;
async function getBudgetTracker() {
  if (budgetTracker) return budgetTracker;
  const core2 = await loadCoreModules();
  if (!core2) return null;
  budgetTracker = core2.createBudgetTracker();
  return budgetTracker;
}
async function getFixLogger() {
  if (fixLogger) return fixLogger;
  const core2 = await loadCoreModules();
  if (!core2) return null;
  fixLogger = await core2.getPersistentFixLogger();
  return fixLogger;
}
function registerWindowHandlers() {
  const handlers = getWindowIpcHandlers();
  electron.ipcMain.handle("window:minimize", () => handlers.minimize());
  electron.ipcMain.handle("window:maximize", () => handlers.maximize());
  electron.ipcMain.handle("window:close", () => handlers.close());
  electron.ipcMain.handle("window:isMaximized", () => handlers.isMaximized());
  electron.ipcMain.handle("window:isFullScreen", () => handlers.isFullScreen());
  electron.ipcMain.handle(
    "window:setFullScreen",
    (_event, fullScreen) => handlers.setFullScreen(fullScreen)
  );
}
function registerDialogHandlers() {
  ...
  electron.ipcMain.handle("dialog:save", async (_event, options) => {
    const win = getMainWindow();
  ...
  electron.ipcMain.handle(
    ...
    async (_event, options) => {
      ...
      return electron.dialog.showMessageBox(win, {
...
function registerSessionHandlers() {
  electron.ipcMain.handle("session:get", async () => {
    const core2 = await loadCoreModules();
    if (!core2) {
      return {
        id: "demo-session",
    ...
    const manager = core2.getSessionManager();
...
function registerBudgetHandlers() {
  electron.ipcMain.handle("budget:get", async () => {
...
function registerCheckpointHandlers() {
  electron.ipcMain.handle("checkpoints:list", async () => {
    const core2 = await loadCoreModules();
    if (!core2) {
      return [];
    }
    try {
      const checkpoints = await core2.listCheckpoints();
      ...
      console.error("[IPC] Failed to list checkpoints:", error2);
      return [];
    }
  });
  electron.ipcMain.handle("checkpoints:restore", async (_event, id2) => {
    const core2 = await loadCoreModules();
    if (!core2) {
      return { success: false, error: "Core modules not available" };
    }
    try {
      const checkpoint = await core2.loadCheckpoint(id2);
      if (!checkpoint) {
      ...
      console.error("[IPC] Failed to restore checkpoint:", error2);
      return { success: false, error: String(error2) };
    }
  });
}
function registerFixStatsHandlers() {
  electron.ipcMain.handle("fixStats:getWeeklySummary", async () => {
    ...
    try {
      const summary = await logger.getWeeklySummary(0);
      const byCategory = {};
      ...
      console.error("[IPC] Failed to get weekly summary:", error2);
      return {
        totalFixes: 0,
        successRate: 0,
        byCategory: {},
  ...
  electron.ipcMain.handle("fixStats:getPatterns", async () => {
    const logger = await getFixLogger();
    if (!logger) {
      return [];
    }
    try {
      const patterns = await logger.getRecurringPatterns(2);
      ...
      console.error("[IPC] Failed to get patterns:", error2);
      return [];
    }
  });
}
    ...
    ...isMac ? [
      {
        ...
        submenu: [
          ...
          { type: "separator" },
          {
            ...
            click: () => navigateTo$1(mainWindow2, "/settings")
          },
          { type: "separator" },
          { role: "services" },
          { type: "separator" },
      ...
      submenu: [
        {
          ...
          click: () => navigateTo$1(mainWindow2, "/chat")
        },
        { type: "separator" },
      ...
      submenu: [
        {
          ...
          click: () => navigateTo$1(mainWindow2, "/dashboard")
        },
        {
          label: "Chat",
        ...
        },
        {
          ...
          click: () => navigateTo$1(mainWindow2, "/checkpoints")
        },
        {
          label: "Fix Stats",
          accelerator: "CmdOrCtrl+4",
          click: () => navigateTo$1(mainWindow2, "/fix-stats")
        },
        {
          label: "Settings",
      ...
      submenu: [
        {
          label: "Documentation",
          click: () => {
            electron.shell.openExternal("https://docs.endiorbot.nqh-internal.example");
          }
        },
        {
          label: "Report Issue",
...
function navigateTo$1(mainWindow2, route) {
  if (mainWindow2) {
    mainWindow2.webContents.send("navigate", route);
  }
}
function setupMenu(mainWindow2) {
  const menu = buildMenu(mainWindow2);
...
function getTrayIconPath() {
  const iconDir = electron.app.isPackaged ? path.join(process.resourcesPath, "icons") : path.join(__dirname, "../../resources/icons");
...
function updateTrayMenu(mainWindow2) {
  ...
  const contextMenu = electron.Menu.buildFromTemplate([
    ...
    {
      label: "Dashboard",
      click: () => navigateTo(mainWindow2, "/dashboard")
    },
    {
      label: "Chat",
      click: () => navigateTo(mainWindow2, "/chat")
    },
    {
      label: "Checkpoints",
      click: () => navigateTo(mainWindow2, "/checkpoints")
    },
    {
      label: "Fix Stats",
      click: () => navigateTo(mainWindow2, "/fix-stats")
    },
    {
      label: "Settings",
      click: () => navigateTo(mainWindow2, "/settings")
    },
    { type: "separator" },
    {
...
function navigateTo(mainWindow2, route) {
  if (mainWindow2) {
    mainWindow2.show();
    mainWindow2.focus();
    mainWindow2.webContents.send("navigate", route);
...
async function initUpdater(win) {
  ...
  try {
    ...
    console.error("Failed to initialize updater:", error2);
  }
}
function setStatus(status, version) {
...
function registerUpdaterIpc() {
  ...
  electron.ipcMain.handle("update:check", async () => {
    ...
    try {
      ...
      return {
        available: (result == null ? void 0 : result.updateInfo) !== void 0,
        version: (_a = result == null ? void 0 : result.updateInfo) == null ? void 0 : _a.version
      };
    } catch (error2) {
      return {
        available: false,
        error: error2 instanceof Error ? error2.message : "Check failed"
      };
    }
  });
  electron.ipcMain.handle("update:download", async () => {
    ...
    try {
      ...
      return {
        success: false,
        error: error2 instanceof Error ? error2.message : "Download failed"
      };
    }
  });
  electron.ipcMain.handle("update:install", async () => {
    ...
    try {
      ...
      return {
        success: false,
        error: error2 instanceof Error ? error2.message : "Install failed"
      };
    }
  });
}
const __filename$1 = node_url.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("index.cjs", document.baseURI).href);
const __dirname$1 = path.dirname(__filename$1);
const isDev = process.env.NODE_ENV === "development" || !electron.app.isPackaged;
const VITE_DEV_SERVER_URL = process.env.VITE_DEV_SERVER_URL ?? "http://localhost:5173";
const PRELOAD_PATH = path.join(__dirname$1, "../preload/index.cjs");
const RENDERER_PATH = path.join(__dirname$1, "../../dist/index.html");
...
electron.ipcMain.handle("app:isDev", () => {
  return isDev;
});
electron.ipcMain.handle("shell:openExternal", (_event, url) => {
  return electron.shell.openExternal(url);
});
electron.ipcMain.handle("shell:showItemInFolder", (_event, fullPath) => {
