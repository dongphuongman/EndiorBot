/**
 * MCP Gateway Cross-System Agent Communication Types
 *
 * Types for EndiorBot ↔ MCP Gateway communication protocol.
 * See ADR-034 for architecture decision.
 *
 * @module mcp-gateway/types
 * @version 1.0.0
 * @date 2026-03-20
...
/**
 * MCP Gateway bridge configuration.
 * Loaded from `.mcp.json` + `MCP_GATEWAY_API_KEY` env var.
 */
export interface McpGatewayConfig {
  /** MCP server URL (from .mcp.json mcpServers config) */
  url: string;
  /** Bearer token — from MCP_GATEWAY_API_KEY env, fallback .mcp.json (CTO C2) */
  authToken: string;
 ...
 * Source: AI-Platform Sprint 60, verified Sprint 92-97.
 */
...
/**
 * MCP Gateway agent info from agent_list tool.
 */
export interface McpGatewayAgentInfo {
...
/** @deprecated Use McpGatewayConfig instead */
export type MTClawConfig = McpGatewayConfig;

/** @deprecated Use McpGatewayAgentInfo instead */
export type MTClawAgentInfo = McpGatewayAgentInfo;
