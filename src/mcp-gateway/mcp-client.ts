...
export function parseToolResult(result: McpToolResult): ToolResult {
  ...
  if (result.isError) {
    return {
      text: "MCP Gateway service encountered an error. Please try again.",
      isError: true,
    };
  }
  ...
  if (!result.content) {
    return {
      text: "No response from MCP Gateway.",
      isError: false,
    };
  }
  ...
  private async sendRequest(
    ...
    if (!response.ok) {
      ...
      if (status === 401 || status === 403) {
        throw new Error(`MCP Gateway authentication failed (${status})`);
      }
      if (status === 429) {
        throw new Error("MCP Gateway rate limit reached. Please wait.");
      }
      throw new Error(`MCP Gateway request failed (HTTP ${status})`);
    }
    ...
    if ("error" in json) {
      throw new Error(
        `MCP Gateway RPC error: ${json.error.message} (code: ${json.error.code})`,
      );
    }
  ...
  private async parseSSEResponse(
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if ((e as Error).message.startsWith("MCP Gateway")) throw e;
