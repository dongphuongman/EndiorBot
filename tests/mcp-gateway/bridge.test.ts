/**
 * MCP Gateway Bridge Tests
 *
 * Tests for McpGatewayBridge: chatWithAgent, searchKnowledge, callTool,
 * graceful degradation, circuit breaker, no-leak (CPO C5).
 *
 * All tests mock HTTP — no real MCP Gateway dependency.
 *
 * @module tests/mcp-gateway/bridge
 * @sprint 113
 */

import { describe, it, expect, beforeEach, vi, afterEach } from "vitest";
import { McpGatewayBridge } from "../../src/mcp-gateway/bridge.js";
import type { McpGatewayConfig } from "../../src/mcp-gateway/types.js";
...
describe("McpGatewayBridge", () => {
  ...
  describe("degradation", () => {
    ...
    it("returns error message on auth failure", async () => {
      fetchMock.mockRejectedValueOnce(new Error("MCP Gateway authentication failed (401)"));
    ...
    it("returns error message on rate limit", async () => {
      fetchMock.mockRejectedValueOnce(new Error("MCP Gateway rate limit reached. Please wait."));
  ...
  describe("security (CPO C5)", () => {
    ...
    it("never exposes stack traces in tool results", async () => {
      ...
      const result = await bridge.callTool("agent_chat", { agent: "pm", message: "test" });
      expect(result.text).toBe("MCP Gateway service encountered an error. Please try again.");
