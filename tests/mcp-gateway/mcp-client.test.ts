 ...
 *
 * All tests mock HTTP — no real MCP Gateway dependency.
 *
 * @module tests/mcp-gateway/mcp-client
 * @sprint 113
 */

import { describe, it, expect, beforeEach, vi, afterEach } from "vitest";
import { McpToolClient, parseToolResult } from "../../src/mcp-gateway/mcp-client.js";
import type { McpGatewayConfig, McpToolResult } from "../../src/mcp-gateway/types.js";

// ============================================================================
// Test Helpers
// ============================================================================

function createTestConfig(overrides?: Partial<McpGatewayConfig>): McpGatewayConfig {
  return {
...
describe("parseToolResult", () => {
  it("C6-1: normal text → extracted text", () => {
    const result: McpToolResult = {
      content: [{ type: "text", text: "Hello from MCP Gateway" }],
    };
    const parsed = parseToolResult(result);
    expect(parsed.text).toBe("Hello from MCP Gateway");
  ...
  it("C6-3: isError flag → user-friendly message", () => {
    ...
    const parsed = parseToolResult(result);
    expect(parsed.text).toBe("MCP Gateway service encountered an error. Please try again.");
  ...
  it("C6-4: empty content array → 'No response' fallback", () => {
    ...
    const parsed = parseToolResult(result);
    expect(parsed.text).toBe("No response from MCP Gateway.");
    expect(parsed.isError).toBe(false);
  });
...
describe("McpToolClient", () => {
  ...
  describe("initialize()", () => {
    ...
    it("throws on HTTP 401", async () => {
      ...
      await expect(client.initialize()).rejects.toThrow("MCP Gateway authentication failed (401)");
    });
    ...
    it("throws on HTTP 429", async () => {
      ...
      await expect(client.initialize()).rejects.toThrow("MCP Gateway rate limit reached");
    });
  });
  ...
  describe("callTool()", () => {
    ...
    it("throws on JSON-RPC error response", async () => {
      ...
      ).rejects.toThrow("MCP Gateway RPC error: Invalid Request");
    });
